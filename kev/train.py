...
def main():
    a = parse_args()
    out_dir = Path(a.out); out_dir.mkdir(parents=True, exist_ok=True)   # exist_ok: torchrun ranks share one out dir
    ...
    if RANK == 0:
        print(f"device={dev} rank={RANK}/{WORLD} trainable params={sum(p.numel() for p in model.trainable_parameters())/1e6:.1f}M", flush=True)
