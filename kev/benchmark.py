...
def data_context():
    """Eval context for --data: CONTEXT lifted by KEV_EVAL_MAX_STATE for augmented states (SRA exemplars).
    Unset (or equal to the training limit) = byte-identical CONTEXT, so existing numbers never move."""
    from kev.model import MAX_BRANCH, MAX_PACKED, MAX_STATE, MAX_TRAIN_STATE
    ms = int(os.environ.get("KEV_EVAL_MAX_STATE", "0") or 0)
    if not ms:
        return dict(CONTEXT)
    if not MAX_STATE <= ms <= MAX_TRAIN_STATE:
        raise ValueError(f"KEV_EVAL_MAX_STATE must be in [{MAX_STATE}, {MAX_TRAIN_STATE}]")
    extra = ms - MAX_STATE
    return {"max_state": ms, "max_branch": MAX_BRANCH + extra, "max_packed": MAX_PACKED + extra, "truncate": False}
...
def main():
    ...
    if a.data:
        ...
        context, skip_overlong = data_context(), True
    else:
    ...
    report.update(suite_sha256=source_hash, data=a.data, date_facts=a.date_facts, rotations=a.rotations, run=a.run or a.remote, split=split,
                  eval_max_state=context["max_state"],
